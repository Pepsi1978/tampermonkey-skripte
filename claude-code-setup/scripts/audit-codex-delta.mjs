...
const DEFAULT_STATE = {
	...
	notes: "Tracks only Codex programming-environment deltas for Claude Code.",
};
...
function getLocalClaudeConfigPath() {
	return toSlash(path.join(os.homedir(), ".claude", "settings.json"));
}
...
function classifyPath(sourcePath) {
	if (sourcePath === "AGENTS.md") {
		return {
			category: "root-rules",
			target_hints: ["CLAUDE.md", "claude-code-setup/rules/"],
		};
	}
	...
	if (/^codex-setup\/rules\/.+\.md$/u.test(sourcePath)) {
		...
		return {
			category: "rule",
			target_hints: [`claude-code-setup/rules/${mappedName}.md`],
		};
	}
	...
	if (sourcePath === "codex-setup/agent-memory/shared/MEMORY.md") {
		return {
			...
			target_hints: [
				...
				"claude-code-setup/state/environment-fixes.json",
	...
	if (
		...
		return {
			...
			target_hints: [
				"claude-code-setup/state/implemented-intelligence-suggestions.json",
				"claude-code-setup/bridges/intelligence-suggestion-exchange-bridge.json",
				"CLAUDE.md",
			],
		};
	}
	...
	if (/^codex-setup\/agents\/.+\.md$/u.test(sourcePath)) {
		return {
			category: "agent",
			target_hints: [`claude-code-setup/agents/${filename}`],
		};
	}
	...
	if (/^codex-setup\/skills\/self-improve/u.test(sourcePath)) {
		return {
			...
			target_hints: [
				"claude-code-setup/commands/self-improve.md",
	...
	if (/^codex-setup\/commands\//u.test(sourcePath)) {
		return {
			...
			target_hints: [
				...
				"claude-code-setup/rules/german-skill-triggers.md",
			],
		};
	}
	...
	if (/^codex-setup\/hooks\//u.test(sourcePath)) {
		return {
			category: "hook",
			target_hints: ["claude-code-setup/scripts/", "CLAUDE.md"],
		};
	}
	...
	if (/^codex-setup\/skills\//u.test(sourcePath)) {
		return {
			category: "skill",
			target_hints: ["claude-code-setup/skills/", "CLAUDE.md"],
		};
	}
	...
	if (/^codex-setup\/scripts\//u.test(sourcePath)) {
		return {
			category: "script",
			target_hints: ["claude-code-setup/scripts/"],
		};
	}
	...
	return {
		category: "setup-doc",
		target_hints: ["claude-code-setup/README.md", "claude-code-setup/skills/"],
	};
}
...
function buildRecommendation(strategy) {
	if (strategy === "ADD") {
		return "Als additive Claude Code-Erweiterung portieren.";
	}
	if (strategy === "REPLACE") {
		return "Vor Ersetzung warnen und ausdrueckliche Freigabe einholen.";
	}
	return "Fuer Claude Code uebersetzen und additive Integration bevorzugen.";
}
...
function buildReason({
	...
	if (strategy === "ADD") {
		const base =
			"Die Aenderung hat keinen direkten Claude Code-Zielkonflikt und wirkt additiv.";
		return bugfixSignal
			? `${base} Zusaetzlich ist robuste Bugfix- oder Haertungslogik erkennbar.`
			: base;
	}
	if (strategy === "REPLACE") {
		const base = `Die Aenderung beruehrt bestehende Claude Code-Ziele (${overlappingTargets.join(", ")}).`;
		return bugfixSignal
			? `${base} Gleichzeitig steckt darin bestehende Fixlogik, die vor einem Ersatz separat bewertet werden soll.`
			: base;
	}
	const base = `Die Aenderung betrifft vorhandene Claude Code-Bereiche (${categories.join(", ")}), braucht aber Uebersetzung statt blindes Spiegeln.`;
	return bugfixSignal
	...
	for (const commit of commits) {
		...
		candidates.push({
			...
			bugfix_recommendation: bugfixSignal
				? "Fixlogik und Regression-Schutz fuer Claude Code mitpruefen und nach Moeglichkeit uebernehmen."
				: "",
			reason: buildReason({
				strategy,
				categories,
...
function formatTextReport(result) {
	...
	for (const group of groups) {
		...
		items.forEach((candidate, index) => {
			...
			lines.push(`  Claude Code-Ziele: ${candidate.target_hints.join(", ")}`);
			lines.push(
			...
			if (candidate.approval_required) {
				lines.push(
					"  Warnung: Diese Aenderung wuerde bestehende Claude Code-Logik beruehren. Vor Ersetzung erst bestaetigen lassen.",
				);
			}
		});
	}
